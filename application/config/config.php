...
|
*/
$config['encryption_key'] = 'Vie Script';
...
if(is_array($uri_segments)){
	for($i=0;$i<count($uri_segments);$i++){
		$uri_segments[$i] = trim($uri_segments[$i]);
		if($uri_segments[$i]==''){
			unset($uri_segments[$i]);
		}
	}
	$uri_segments = array_values($uri_segments);
	if(count($uri_segments)==2 && in_array('auth',$uri_segments) && in_array('login',$uri_segments) && $_POST['remember']==1){
