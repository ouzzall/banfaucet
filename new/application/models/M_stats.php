...
class M_Stats extends CI_Model
{
	...
	public function countPtc()
	{
		$lastView = $this->db->query("SELECT * FROM ptc_history ORDER BY id DESC LIMIT 1");
		if ($lastView->num_rows() == 0) {
			return 0;
		}
		return $lastView->result_array()[0]['id'];
	}
	...
	{
		return $this->db->query('SELECT users.username, currencies.code, withdraw_history.* FROM users, withdraw_history, currencies WHERE withdraw_history.user_id=users.id AND withdraw_history.type = 1 AND currencies.id = withdraw_history.method ORDER BY id DESC LIMIT 15')->result_array();
	}
